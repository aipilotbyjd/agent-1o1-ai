...
## Standing up without Workflows/Agents (implementation status: built)

This system is now implemented in full against this plan — migrations, models, `TriggerService`, `WebhookSignatureVerifier`, `FireTriggerEvent`/`PollTrigger` jobs, the webhook + Internal CRUD HTTP layer, both console commands, the preset seeder, and Horizon supervisors — with one deliberate seam, decided before implementation started: **`docs/WORKFLOWS_PLAN.md`/`docs/AGENTS_PLAN.md` don't exist yet**, so `TriggerService::fire()` doesn't call `StartWorkflowRunAction`/`AgentRunner` directly. Instead:

- **`App\Contracts\Triggers\RunStarter`** (`canRun()`, `isAlreadyRunning()`, `start()`) is the seam. `App\Services\Triggers\NullRunStarter` is the only implementation today — bound in `AppServiceProvider::register()` — and stands in until either real target exists: `canRun()` always returns `true` (no way to check "workflow is published" without the model), `isAlreadyRunning()` always returns `false`, and `start()` returns a synthetic id so the rest of the pipeline (claim → fire → mark fired) runs for real rather than being mocked out. Swap the binding for a real implementation the day `StartWorkflowRunAction` or `AgentRunner` ships; nothing else in the trigger system changes.
- **`triggers.target_type`/`target_id`** use a morph map (`TriggerTargetType::Workflow` → `App\Models\Workflows\Workflow`, `TriggerTargetType::Agent` → `App\Models\Agents\Agent`, registered in `AppServiceProvider::configureMorphMap()`) referencing classes that don't exist yet — safe, since `::class` resolves to a string at compile time without autoloading.
- **`triggers.credential_id`** and **`trigger_events.run_id`** are real columns without FK constraints yet (`connector_credentials` and `runs` don't exist) — the constraints follow once those tables ship. **Follow-up needed**: the already-migrated `2026_08_08_195933_create_trigger_events_table.php` still names this column `workflow_run_id`; a new migration should rename it to `run_id` once `docs/WORKFLOWS_PLAN.md`'s `runs` table lands, since the column is now polymorphic (points at a workflow *or* agent-session run) rather than workflow-only.
